...
function HomePanel() {
  ...
  const tutorial = [
    ...
    '게임의 순서는 다음과 같이 진행됩니다 <br /> <br />1) 아홉 개의 종목 중에서 가장 상승할 것으로 예상되는 세 개의 코인을 선택하세요<br />2) 각 종목에 투자할 금액을 선택하세요 <br />3) 실시간으로 변동하는 코인 시세를 보고, 적절한 매도 타이밍으로 최대한의 이익을 남기세요!',
    <>
      ...
      <div css={hintTextCss}>
        <HintTag hint={'사용자픽'} /> - 플레이어들이 가장 많이 선택한 종목
      </div>
    </>,
    '우선, 아홉 개의 비트코인 종목 중에서, 가장 상승할 것으로 예상되는 세 개의 그래프를 골라주세요<br />제한 시간은 90초 입니다<br />선택이 어려우시다면 AI 추천을 활용해보세요!<br/>(주의: 공매도 불가)',
  ];
